...
const stepOneResetBtn = document.getElementById("stepOneResetBtn");
const smartAssistBtn = document.getElementById("smartAssistBtn");
const smartAssistStatus = document.getElementById("smartAssistStatus");
...
let isApplyingSmartAssist = false;

const smartAssistTargetFields = [
  "coreFeatures",
  "modulesScreens",
  "userRoles",
  "workflow",
...
const smartAssistStatusDefaults = {
  idle: "Isi maklumat asas projek. Sistem akan cadangkan bahagian Modul, Data dan Teknikal secara automatik, dan anda masih boleh ubah manual.",
  ready: "Cadangan pintar aktif. Medan kosong dalam Modul, Data dan Teknikal akan diisi automatik berdasarkan maklumat asas projek.",
  manual: "Beberapa medan telah anda ubah sendiri. Sistem tidak akan menimpa medan itu kecuali anda klik Jana Cadangan Pintar.",
  forced: "Cadangan pintar dijana semula berdasarkan maklumat asas projek. Anda masih boleh ubah semua cadangan ini secara manual."
};
...
function resetWorkspaceControls() {
  ...
  clearDeployBuilder();
  clearExtractor();
  clearSmartAssistState();
}

function setSmartAssistStatus(message) {
  if (smartAssistStatus) {
    smartAssistStatus.textContent = message;
  }
}
...
function clearSmartAssistState() {
  smartAssistTargetFields.forEach((name) => {
    const field = getNamedFormField(name);

    if (!field?.dataset) {
      return;
    }

    delete field.dataset.autoGenerated;
    delete field.dataset.userEdited;
  });

  setSmartAssistStatus(smartAssistStatusDefaults.idle);
}
...
function toMultilineValue(items) {
  return dedupeList(items).join("\n");
}

function getSmartAssistRecipe(values) {
  const context = `${values.projectName} ${values.mainGoal} ${values.problemSolved}`.toLowerCase();

  if (/\b(kehadiran|hadir|attendance|qr)\b/.test(context)) {
    return {
      label: "kehadiran",
      features: [
        "rekod hadir masuk dan keluar",
        "status kehadiran masa nyata",
        "import data pengguna atau jadual",
        "carian dan penapis rekod",
        "notifikasi atau amaran lewat hadir",
        "laporan automatik"
      ],
      modules: [
        "dashboard kehadiran",
      ...
      entities: ["pengguna", "rekod kehadiran", "status kehadiran", "notifikasi", "laporan"],
      reports: ["laporan harian", "laporan bulanan", "laporan kelewatan", "eksport PDF atau Excel"],
      integrations: ["CSV atau Excel import", "QR scanner", "emel atau WhatsApp notifikasi"]
    };
  }

  if (/\b(kokurikulum|kelab|sukan|uniform)\b/.test(context)) {
    return {
      label: "kokurikulum",
      features: [
        "pendaftaran aktiviti dan peserta",
        "rekod kehadiran aktiviti",
        "markah atau merit automatik",
        "pengumuman dan jadual aktiviti",
        "laporan penglibatan"
      ],
      modules: [
        ...
        "modul aktiviti",
        "profil peserta",
        "rekod kehadiran dan markah",
        "laporan dan eksport"
      ],
      entities: ["aktiviti", "peserta", "guru penasihat", "kehadiran", "markah", "laporan"],
      reports: ["laporan kehadiran aktiviti", "laporan merit", "ringkasan penglibatan", "eksport PDF"],
      integrations: ["Google Calendar", "emel notifikasi", "CSV atau Excel import"]
    };
  }

  if (/\b(pajsk)\b/.test(context)) {
    return {
      label: "PAJSK",
      features: [
        "rekod skor PAJSK",
        "pengiraan markah automatik",
        "profil murid dan kelas",
        "import data murid",
      ...
      entities: ["murid", "kelas", "aktiviti", "skor PAJSK", "guru", "laporan"],
      reports: ["laporan PAJSK murid", "laporan kelas", "ringkasan pencapaian", "eksport PDF"],
      integrations: ["CSV atau Excel import"]
    };
  }

  if (/\b(buku|perpustakaan|pusat sumber|pinjaman)\b/.test(context)) {
    return {
      label: "pinjaman buku",
      features: [
        "rekod inventori dan stok",
        "pinjaman dan pemulangan",
        "carian buku atau item",
        "amaran lewat pulang",
        "laporan penggunaan"
      ],
      modules: [
      ...
      entities: ["item", "kategori", "peminjam", "rekod pinjaman", "rekod pemulangan", "stok"],
      reports: ["laporan pinjaman", "item lewat pulang", "item popular", "eksport PDF atau Excel"],
      integrations: ["barcode scanner", "CSV atau Excel import"]
    };
  }

  if (/\b(dashboard|analitik|analytics|prestasi|kpi)\b/.test(context)) {
    return {
      label: "dashboard",
      features: [
        "paparan KPI utama",
        "graf dan trend interaktif",
        "penapis ikut kelas, tarikh atau kategori",
        "drill-down data",
        "laporan dan eksport"
      ],
      modules: [
      ...
      entities: ["pengguna", "metrik", "kategori data", "rekod analitik", "laporan"],
      reports: ["ringkasan KPI", "laporan trend", "laporan perbandingan", "eksport PDF atau Excel"],
      integrations: ["Google Sheets", "CSV atau Excel import"]
    };
  }

  if (/\b(murid|pelajar|akademik|disiplin|intervensi)\b/.test(context)) {
    return {
      label: "rekod murid",
      features: [
        "profil lengkap murid",
        "rekod akademik, disiplin dan intervensi",
        "carian dan penapis murid",
        "import data murid",
        "laporan individu dan kelas"
      ],
      modules: [
        ...
        "senarai murid",
        "profil murid",
        "rekod akademik dan disiplin",
        "laporan dan eksport"
      ],
      entities: ["murid", "kelas", "guru", "akademik", "disiplin", "intervensi", "laporan"],
      reports: ["laporan murid individu", "laporan kelas", "laporan murid berisiko", "eksport PDF atau Excel"],
      integrations: ["Google Sheets", "CSV atau Excel import"]
    };
  }
  ...
  return {
    ...
    features: [
      ...
      "carian dan penapis pantas",
      "borang tambah, semak dan kemas kini rekod",
      "dashboard ringkas",
      "laporan dan eksport"
    ],
    modules: [
      "dashboard utama",
      "senarai rekod",
      "borang tambah atau kemas kini",
      "laporan dan eksport",
      "tetapan sistem"
    ],
    entities: ["pengguna", "rekod utama", "kategori", "status", "laporan"],
    reports: ["laporan harian", "laporan bulanan", "ringkasan prestasi", "eksport PDF atau Excel"],
    integrations: ["CSV atau Excel import", "Google Sheets", "emel notifikasi"]
  };
}

function inferOutputSpecificModules(values) {
  const outputType = String(values.outputType || "").toLowerCase();

  if (outputType.includes("dashboard")) {
    return ["graf dan penapis interaktif"];
  }

  if (outputType.includes("portal")) {
    return ["halaman utama portal", "profil pengguna", "pengumuman atau notis"];
  }

  if (outputType.includes("mudah alih")) {
    return ["paparan mobile-friendly", "tindakan pantas pada skrin utama"];
  }

  return ["paparan ringkasan utama", "pengurusan data asas"];
}

function buildRoleSuggestions(values) {
  const roles = normalizeList(values.targetUsers || "");
  const hasAdminRole = roles.some((role) => /\b(admin|pentadbir|pengetua|pengurus)\b/i.test(role));
  const suggestedRoles = hasAdminRole ? roles : ["pentadbir sistem", ...roles];

  return toMultilineValue(suggestedRoles.length ? suggestedRoles : ["pentadbir sistem", "pengguna utama"]);
}

function buildWorkflowSuggestion(values, roles, recipe) {
  const primaryRole = roles.find((role) => !/\b(admin|pentadbir|pengetua|pengurus)\b/i.test(role)) || roles[0] || "pengguna utama";
  const adminRole = roles.find((role) => /\b(admin|pentadbir|pengetua|pengurus)\b/i.test(role)) || "pentadbir sistem";
  const projectLabel = values.projectName || "sistem ini";
  const focusLabel = recipe.label === "projek umum" ? "rekod utama" : recipe.label;

  return [
    `${adminRole} menyediakan konfigurasi asas, kategori dan data rujukan untuk ${projectLabel}.`,
    `${primaryRole} menambah, mengemas kini atau mengimport data berkaitan ${focusLabel} melalui borang atau fail CSV/Excel.`,
    "Sistem menyemak input, menyimpan rekod, memaparkan status semasa dan mengemas kini paparan ringkasan secara automatik.",
    `${adminRole} atau pengguna berkaitan menapis data, menyemak laporan dan mengeksport rumusan untuk tindakan susulan.`
  ].join("\n");
}

function buildEntitySuggestions(recipe) {
  const mappedHeaders = (datasetAnalysis.headers || []).slice(0, 6).map((header) => header.toLowerCase());

  return toMultilineValue([
    ...recipe.entities,
    ...mappedHeaders,
    datasetAnalysis.fileName ? "rekod import" : "",
    "audit log"
  ]);
}

function buildIntegrationSuggestions(recipe) {
  return toMultilineValue([
    ...recipe.integrations,
    datasetAnalysis.fileName ? `${datasetAnalysis.fileType} import dengan preview data` : "",
    "eksport PDF atau Excel"
  ]);
}

function buildSecuritySuggestion(values) {
  if (hasExplicitAuthenticationRequest(values)) {
    return "login selamat, role-based access, validasi input, audit trail, backup data dan perlindungan data sensitif";
  }

  return "tiada login diperlukan, validasi input, audit trail, backup data, kawalan akses pada operasi sensitif dan perlindungan data";
}

function buildTechStackSuggestion(values) {
  const outputType = String(values.outputType || "").toLowerCase();
  const platform = String(values.platform || "").toLowerCase();
  const stack = [];

  if (outputType.includes("dashboard")) {
    stack.push("HTML, CSS, JavaScript", "Chart.js");
  } else if (outputType.includes("mudah alih")) {
    stack.push("Flutter atau React Native", "web admin dengan HTML, CSS, JavaScript");
  } else {
    stack.push("HTML, CSS, JavaScript");
  }

  if (datasetAnalysis.fileName) {
    stack.push("SheetJS/XLSX untuk import Excel");
  }

  if (platform.includes("desktop")) {
    stack.push("Electron atau Tauri jika perlukan aplikasi desktop");
  }

  stack.push("Firebase atau Supabase");
  return stack.join(", ");
}

function buildDesignStyleSuggestion(values) {
  const tone = /\bguru\b/i.test(values.targetUsers || "") ? "mesra guru" : "mesra pengguna";
  const outputType = String(values.outputType || "").toLowerCase();

  if (outputType.includes("dashboard")) {
    return `moden, profesional, visual data jelas, ${tone}, fokus pada jadual dan graf`;
  }

  return `moden, kemas, jelas, ${tone}, fokus pada borang, jadual dan tindakan utama`;
}

function buildConstraintSuggestion(values) {
  const platform = String(values.platform || "").toLowerCase();
  const mobileNeed = platform.includes("mudah alih") || platform.includes("tablet") ? "mesra skrin sentuh" : "responsif pada desktop dan telefon";

  return `mudah digunakan oleh pengguna bukan teknikal, ${mobileNeed}, aliran kerja ringkas, bahasa yang jelas, kos rendah dan mudah diselenggara`;
}

function hasEnoughSmartAssistContext(values) {
  const coreSignals = [
    values.projectName,
    values.targetUsers,
    values.mainGoal,
    values.problemSolved
  ].filter((value) => String(value || "").trim().length >= 4);

  return coreSignals.length >= 2;
}

function buildSmartAssistSuggestions(values) {
  const recipe = getSmartAssistRecipe(values);
  const rolesList = normalizeList(buildRoleSuggestions(values));

  return {
    coreFeatures: toMultilineValue([
      ...recipe.features,
      datasetAnalysis.fileName ? "preview data sebelum import" : "",
      "carian, penapis dan tindakan pantas"
    ]),
    modulesScreens: toMultilineValue([
      ...recipe.modules,
      ...inferOutputSpecificModules(values)
    ]),
    userRoles: toMultilineValue(rolesList),
    workflow: buildWorkflowSuggestion(values, rolesList, recipe),
    dataEntities: buildEntitySuggestions(recipe),
    reports: toMultilineValue(recipe.reports),
    integrations: buildIntegrationSuggestions(recipe),
    security: buildSecuritySuggestion(values),
    techStack: buildTechStackSuggestion(values),
    designStyle: buildDesignStyleSuggestion(values),
    constraints: buildConstraintSuggestion(values)
  };
}

function applySmartAssistField(name, value, force = false) {
  const field = getNamedFormField(name);
  ...
  const wasAutoGenerated = field.dataset.autoGenerated === "true";
  const wasUserEdited = field.dataset.userEdited === "true";

  if (!nextValue) {
    return false;
  }

  if (!force && wasUserEdited && currentValue) {
    return false;
  }

  if (!force && currentValue && !wasAutoGenerated) {
    return false;
  }
...
function autoFillProjectSections(options = {}) {
  const { force = false } = options;
  const values = readFormData();

  if (!hasEnoughSmartAssistContext(values)) {
    setSmartAssistStatus(smartAssistStatusDefaults.idle);
    return 0;
  }

  const suggestions = buildSmartAssistSuggestions(values);
  let updatedCount = 0;

  isApplyingSmartAssist = true;

  try {
    smartAssistTargetFields.forEach((name) => {
      if (applySmartAssistField(name, suggestions[name], force)) {
        updatedCount += 1;
      }
    });
  } finally {
    isApplyingSmartAssist = false;
  }

  if (force) {
    setSmartAssistStatus(smartAssistStatusDefaults.forced);
  } else if (updatedCount > 0) {
    setSmartAssistStatus(`Cadangan pintar telah mengisi ${updatedCount} medan secara automatik. Anda masih boleh ubah semua nilai ini secara manual.`);
  } else {
    const hasManualFields = smartAssistTargetFields.some((name) => getNamedFormField(name)?.dataset.userEdited === "true");
    setSmartAssistStatus(hasManualFields ? smartAssistStatusDefaults.manual : smartAssistStatusDefaults.ready);
  }
...
function handleFormInput(event) {
  ...
  if (
    !isApplyingSmartAssist
    && target?.name
    && smartAssistTargetFields.includes(target.name)
    && "value" in target
  ) {
    if (String(target.value || "").trim()) {
      target.dataset.userEdited = "true";
      target.dataset.autoGenerated = "false";
      setSmartAssistStatus(smartAssistStatusDefaults.manual);
    } else {
      delete target.dataset.userEdited;
      delete target.dataset.autoGenerated;
    }
  }
...
stepOneResetBtn.addEventListener("click", resetStepOneForm);
smartAssistBtn?.addEventListener("click", () => {
  if (!hasEnoughSmartAssistContext(readFormData())) {
    setSmartAssistStatus("Lengkapkan sekurang-kurangnya nama projek, pengguna utama, matlamat atau masalah supaya cadangan pintar boleh dijana.");
    getProjectNameField()?.focus();
    return;
  }

  autoFillProjectSections({ force: true });
  generatePrompt();

  const originalText = smartAssistBtn.textContent;
  smartAssistBtn.textContent = "Cadangan Dijana";

  window.setTimeout(() => {
    smartAssistBtn.textContent = originalText;
  }, 1400);
});
